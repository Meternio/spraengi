...
const Card: React.FC<{ blok: CardStoryblok; isLoading?: boolean, fullWidth?: boolean }> = ({
  blok,
  isLoading = false,
  fullWidth = true,
}) => {
  ...
  return (
    ...
    >
      {blok.image && (
        <Image
          src={String(`${blok.image.filename}/m/${fullWidth ? '1600x0' : '800x0'}/filters:quality(75)`)}
        ...
        {blok.variant === "event" && (
          <div className="text-white flex flex-col h-full gap-4">
            <p className="text-sm mb-2">
              {blok.date ? new Date(blok.date).toLocaleDateString("de-DE") : "Kein Datum"}
            </p>
